...
### Loading stopwords from commonwordsES.cvs
...
t0 = time()
if opts.use_hashing:
    vectorizer = HashingVectorizer(stop_words=stopwordsES, non_negative=True,
                                   decode_error='ignore', n_features=opts.n_features)
    X_train = vectorizer.transform(data_train_data)
else:
    vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=0.5, ngram_range=(1,4),
                                   decode_error='ignore', stop_words=stopwordsES)
